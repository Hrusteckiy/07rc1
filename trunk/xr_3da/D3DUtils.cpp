...
void CDrawUtilities::OnDeviceCreate()
{
	...
	m_Font						= xr_new<CGameFont>("stat_font", (u32)0, false);
}
...
void CDrawUtilities::DrawFlag(const Fvector& p, float heading, float height, float sz, float sz_fl, u32 clr, BOOL bDrawEntity){
	...
	if (bDrawEntity){
		...
		sz				*= 0.8f;
		_pv->set		(p.x,p.y+height,p.z,clr);											_pv++;
		_pv->set		(p.x+rx*sz,p.y+height,p.z+rz*sz,clr);								_pv++;
		sz				*= 0.5f;
		_pv->set		(p.x,p.y+height*(1.f-sz_fl*.5f),p.z,clr);							_pv++;
		_pv->set		(p.x+rx*sz*0.6f,p.y+height*(1.f-sz_fl*.5f),p.z+rz*sz*0.75f,clr);	_pv++;
		_pv->set		(p.x,p.y+height*(1.f-sz_fl),p.z,clr);								_pv++;
		_pv->set		(p.x+rx*sz,p.y+height*(1.f-sz_fl),p.z+rz*sz,clr);					_pv++;
		Stream->Unlock	(6,vs_L->vb_stride);
		// and Render it as line list
		DU_DRAW_DP		(D3DPT_LINELIST,vs_L,vBase,3);
	}else{
