...
char* TCHARToChar(const TCHAR* tstr)
{
	if (!tstr) return NULL;
#ifdef UNICODE
	int size = WideCharToMultiByte(CP_ACP, 0, tstr, -1, NULL, 0, NULL, NULL);
	char* str = (char*)malloc(size);
	WideCharToMultiByte(CP_ACP, 0, tstr, -1, str, size, NULL, NULL);
	return str;
#else
	return _strdup(tstr);
#endif
...
void getDefaultPlaybackDeviceNames(char* longName, char* shortName, unsigned int len)
{
	...
	if (bFoundOutputName == false)
	{
		...
		char* temp = TCHARToChar(outputInfo.szPname);
		if ((shortName != NULL) && (temp != NULL) && (strlen(temp) <= len))
		{
...
void getDefaultCaptureDeviceNames(char *longName, char *shortName, unsigned int len)
{
	...
	if (bFoundInputName == false)
	{
		...
		#endif
		#endif // !defined(_WIN64)
		char* temp = TCHARToChar(inputInfo.szPname);
		if ((shortName != NULL) && (temp != NULL) && (strlen(temp) <= len))
		{
			strcpy(shortName, temp);
		}
